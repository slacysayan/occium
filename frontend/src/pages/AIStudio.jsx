...
import { Wand2, Image as ImageIcon, Copy, Loader2, Sparkles, Mic, PlayCircle, Save } from 'lucide-react';
...
const AIStudio = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            if (mode === 'text') {
                // Determine tone from selected voice
                const tone = voices.find(v => v.id === selectedVoice)?.name.toLowerCase() || "professional";
                ...
                toast.success("Content generated!");
                 ...
                 // Mocking Image/Voice for now
                 await new Promise(r => setTimeout(r, 2000));
                 setGeneratedText("[MOCK] Generated result would appear here. Feature coming soon.");
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-3 space-y-4">
                    <button 
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-4 rounded-xl transition-all border ${mode === 'text' ? 'bg-white text-black border-white' : 'bg-black/20 text-white/60 border-transparent hover:bg-white/5 hover:text-white'}`}
                    >
                        <Wand2 size={20} /> Ghostwriter
                    </button>
                    <button 
                        onClick={() => setMode('image')}
                        className={`w-full flex items-center gap-3 px-4 py-4 rounded-xl transition-all border ${mode === 'image' ? 'bg-white text-black border-white' : 'bg-black/20 text-white/60 border-transparent hover:bg-white/5 hover:text-white'}`}
                    >
                        <ImageIcon size={20} /> Thumbnail Gen
                    </button>
                    <button 
                        onClick={() => setMode('voice')}
                        className={`w-full flex items-center gap-3 px-4 py-4 rounded-xl transition-all border ${mode === 'voice' ? 'bg-white text-black border-white' : 'bg-black/20 text-white/60 border-transparent hover:bg-white/5 hover:text-white'}`}
                    >
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {voices.map(v => (
                                            <button 
                                                ...
                                                className={`p-3 rounded-lg border text-left transition-all ${selectedVoice === v.id ? 'bg-occium-gold/20 border-occium-gold text-white' : 'bg-white/5 border-white/10 text-white/50 hover:border-white/30'}`}
                                            >
                                                <div className="font-medium text-sm mb-1">{v.name}</div>
                                                <div className="text-[10px] opacity-70 leading-tight">{v.desc}</div>
                                            </button>
                                        ))}
                                    </div>
